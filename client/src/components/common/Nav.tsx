...
const Nav = ({
  ...
  const getBorderClass = () => {
    ...
    if (borderOnMobileOnly) {
      const mobileClasses = 'border-b border-gray-100 pb-1 md:pb-4';
    ...
    return 'border-b border-gray-100 pb-1 md:pb-4';
  };
  ...
  return (
    <nav
      className={`w-full px-6 transition-all duration-300 ${getBorderClass()} ${
        isScrolled
          ? 'fixed top-0 right-0 left-0 z-50 border-gray-200 bg-white/100 backdrop-blur-[3.8px]'
      ...
      <div className={`mx-auto flex max-w-7xl items-center justify-between py-2 md:py-4`}>
        ...
        <Button
          ...
          className={`relative z-[60] ${
            isMenuOpen
              ? 'flex h-12 w-12 items-center justify-center rounded-full bg-white/90 shadow-lg backdrop-blur-sm'
